...
describe("MenuComponent", () => {
  ...
  it("carries a rename made while the switch's save is out, and does not let the stale response undo it", () => {
    // A rename saves through the menu's own persistWorkflow, queued behind the switch's save, and
    // the full-page load would abort it. The hand-over saves once more (so it leaves only after the
    // rename's save has completed) and does not apply the switch's response, which still carries
    // the old name, over the new one.
    component.writeAccess = true;
    ...
    const metadataSpy = vi
      .spyOn(component["workflowActionService"], "setWorkflowMetadata")
      .mockImplementation(() => {});
    const switchSave$ = new Subject<any>();
    const renameSave$ = new Subject<any>();
    const resave$ = new Subject<any>();
    const persistSpy = vi
      .spyOn(workflowPersistService, "persistWorkflow")
      .mockReturnValueOnce(switchSave$)
      .mockReturnValueOnce(renameSave$)
      .mockReturnValueOnce(resave$);
    const navigate = vi.spyOn(component as any, "openFormViewPage").mockImplementation(() => {});

    component.onClickOpenFormView();
    component.currentWorkflowName = "new";
    component.onWorkflowNameChange(); // while the switch's save is out
    const stale = { wid: 7, name: "old" } as any;
    switchSave$.next(stale);
    switchSave$.complete();

    expect(metadataSpy).not.toHaveBeenCalledWith(stale);
    expect(persistSpy).toHaveBeenCalledTimes(3); // the rename's save, then the hand-over's once more
    expect(navigate).not.toHaveBeenCalled();
    renameSave$.complete();
    resave$.complete();
    expect(navigate).toHaveBeenCalledWith(7);
