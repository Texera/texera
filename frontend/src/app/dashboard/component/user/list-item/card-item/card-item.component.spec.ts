...
describe("CardItemComponent", () => {
  ...
  describe("default view", () => {
    ...
    // A collaborator with write access but no ownership has no image controls, yet still gets the
    // toggle, so the hover controls must show for them too.
    it("shows the cover controls for a writer who does not own the card", () => {
      enableFormView();
      component.entry = makeWorkflowEntry({
        id: 7,
        accessibleUserIds: [42],
      ...
      expect(component.canEditCover).toBe(false);
      expect(component.canToggleDefaultView).toBe(true);
      expect(component.hasCoverControls).toBe(true);
    });
    ...
    it("renders the toggle for a writer and routes its click, but not for a reader", () => {
      ...
      const button = fixture.debugElement.query(By.css("button.default-view-toggle"));
